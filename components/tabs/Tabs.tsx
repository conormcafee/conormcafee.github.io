...
export const Tabs = () => {
  ...
  const getListStyles = (triggerValue: string): string => {
    ...
    return triggerValue === value ? `${defaultTriggerStyles} ${activeTrigger}` : `${defaultTriggerStyles} ${defaultTrigger}`
  }

  return (
    <ReactTabs.Root defaultValue={value} orientation="horizontal" className='bg-slate-800 border border-slate-700 rounded-sm'>
      <ReactTabs.List aria-label="bio tabs" className='border-b border-slate-700 text-sm'>
        {tabs.map(({ title, id }) => {
          return <ReactTabs.Trigger key={id} value={id} className={getListStyles(id)} onClick={() => setValue(id)}>{title}</ReactTabs.Trigger>
        })}
      </ReactTabs.List>
      <TabsContent />
    </ReactTabs.Root>
